...
###############################
##################### CC3M
OUTPATH=/raid/sedigheh.eslami/outputs/alignCLIP_openCLIP/logs/cc3m
TRAIN_DATA=/raid/sedigheh.eslami/datasets/cc3m/Train_GCC-training_output.tsv
PROJECT_NAME=open_clip_CC3M_kisz

python -m training.main --logs=$OUTPATH --save-frequency 2 --report-to wandb --wandb-project-name=$PROJECT_NAME --train-data=$TRAIN_DATA --warmup 10000  --batch-size=$BS --lr=$LR --wd=0.1 --epochs=$N_EPOCHS --workers=2 --model $MODEL --precision amp 